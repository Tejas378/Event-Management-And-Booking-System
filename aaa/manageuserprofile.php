...
<div id = "my2"  style=" display: none; margin-left: 210px; ">
	               ...
	               <?php
	                   $bb = $_SESSION['user'];
                        ...
                        $n = pg_fetch_row($m);
				    $b  = pg_query("select * from booking where userid = $n[0] and cid = '-1' ; ");
				    $row = pg_fetch_row($b);
				    
				    $row = pg_fetch_row($b);
                         while ($row)
                          {
                          	$a = pg_query("select * from $row[1] where eventid = $row[3] ; ");
                          	$ro=pg_fetch_row($a);
                          	echo ' <div> ';
				                  echo ' <div style="width: 32%; float:left;"> '
				                  		. ' <img src="images/wedding2.jpg" > '
				                  	     . ' </div> ';
				                  echo '<div style="width: 66%;  float:left;">  <h1> ' . $ro[1] . ' <h1>   ';
				                  echo ' <p class="pp"> ' . $ro[2] . '</p>';                    
				                  echo '<h3> Menus</h3>' . $ro[3] . '<br>';
				                  echo '<h3> Cost  = ' . $ro[5] . '</h3>';
				                  echo " <b> Status =  Pending </b><br><br>";
				                  echo '<button onclick = "cancel(\''.$row[1].'\',\''.$row[3].'\''.')" class = "bttn w3-green">Cancel Booking</button> ' ;
				                  echo ' <hr> ' ;
							   echo ' </div> ' ;
			         		echo ' </div> ' ;
                            $row=pg_fetch_row($b);
				    ...
				    $row = pg_fetch_row($b);
                         while ($row)
                          {
                          	$a = pg_query("select * from $row[1] where eventid = $row[2] ; ");
                          	$ro=pg_fetch_row($a);
                          	echo ' <div> ';
				                  echo ' <div style="width: 32%; float:left;"> '
				                  		. ' <img src="images/wedding2.jpg" > '
				                  ...
				                  echo '<h3> Cost  = ' . $ro[5] . '</h3>';
				                  echo " <b> Status =  Pending </b><br><br>";
				                  echo '<button onclick = "cancel(\''.$row[1].'\',\''.$row[2].'\''.')" class = "bttn w3-green">Cancel Booking</button> ' ;
				                  echo ' <hr> ' ;
							   echo ' </div> ' ;
			         		echo ' </div> ' ;
                            $row=pg_fetch_row($b);
